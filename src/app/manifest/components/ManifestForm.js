...
export default function ManifestForm({ onClose, onSuccess }) {
  ...
  const [form, setForm] = useState({
    ...
    // App Architecture (Local state mainly, but could be passed if API supported it)
  ...
  const renderStep1 = () => (
    <div className="space-y-4 animate-in fade-in slide-in-from-right-4 duration-300">
        <div className="flex items-center gap-2 mb-4 text-[#FFA500]">
            <Box size={24} />
            <h2 className="font-bold uppercase text-sm tracking-wider text-neutral-500 dark:text-neutral-400">Application Identity</h2>
        </div>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="md:col-span-2">
                ...
                value={form.appName}
                onChange={e => setForm({...form, appName: e.target.value.toLowerCase().replace(/\s+/g, '-')})}
                />
                <p className="text-[10px] text-neutral-400 mt-1">Lowercase, no spaces. Will be used for DNS.</p>
            </div>
            
            <div className="md:col-span-2">
                <label className="block text-xs font-bold text-neutral-500 uppercase mb-2">Docker Image Repository</label>
                <div className="relative">
                    ...
                    />
                    <Box className="absolute left-3 top-3 text-neutral-400" size={16} />
                </div>
            </div>

            <div className="md:col-span-2">
                <label className="block text-xs font-bold text-neutral-500 uppercase mb-2">Image Tag / Version</label>
                <input required 
                className="w-full p-3 text-sm border rounded-lg dark:bg-neutral-950 dark:border-neutral-800 focus:ring-2 focus:ring-[#FFA500] outline-none transition-all"
  ...
  const renderStep2 = () => (
    <div className="space-y-6 animate-in fade-in slide-in-from-right-4 duration-300">
        <div className="flex items-center gap-2 mb-4 text-[#FFA500]">
            <Database size={24} />
            <h2 className="font-bold uppercase text-sm tracking-wider text-neutral-500 dark:text-neutral-400">Database Configuration (Optional)</h2>
        </div>
            ...
            {['none', 'postgres', 'mariadb'].map((type) => (
                <div 
                    ...
                    className={`cursor-pointer rounded-xl border-2 p-4 flex flex-col items-center justify-center gap-3 transition-all ${ 
                        form.dbType === type 
                        ? 'border-[#FFA500] bg-[#FFA500]/5 dark:bg-[#FFA500]/10' 
                ...
                >
                    ...
                    <span className="font-bold capitalize text-sm text-neutral-700 dark:text-neutral-300">
                        {type === 'none' ? 'No Database' : type === 'mariadb' ? 'MariaDB / MySQL' : 'PostgreSQL'}
                    </span>
  ...
  const renderStep3 = () => (
    <div className="space-y-6 animate-in fade-in slide-in-from-right-4 duration-300">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div 
                onClick={() => setForm({...form, appArchitecture: 'monolith'})}
                className={`cursor-pointer rounded-xl border-2 p-6 flex items-start gap-4 transition-all ${ 
                    form.appArchitecture === 'monolith' 
                    ? 'border-[#FFA500] bg-[#FFA500]/5 dark:bg-[#FFA500]/10' 
                    : 'border-neutral-200 dark:border-neutral-800 hover:border-neutral-300 dark:hover:border-neutral-700'
                }`}
            >
                ...
                <div>
                    ...
                    <p className="text-xs text-neutral-500 mt-1 leading-relaxed">
                        Single deployment unit. Ideal for standard web apps, simple APIs, or legacy systems.
                    </p>
                </div>
            ...
            >
                ...
                <div>
                    <h3 className="font-bold text-neutral-800 dark:text-neutral-200 flex items-center gap-2">
                        Microservices <span className="text-[10px] bg-neutral-200 dark:bg-neutral-800 px-2 py-0.5 rounded text-neutral-500">SOON</span>
                    </h3>
                    <p className="text-xs text-neutral-500 mt-1 leading-relaxed">
                        Distributed system with multiple services. Feature coming in future updates.
                    </p>
                </div>
            </div>
        </div>
    ...
    <div className="space-y-8 animate-in fade-in slide-in-from-right-4 duration-300">
        ...
        <div className="grid grid-cols-2 gap-4">
             <div>
                  ...
                  />
                  <p className="text-[10px] text-neutral-400 mt-1">Exposed internal port (e.g. 80)</p>
            </div>
            <div>
                  ...
                  />
                  <p className="text-[10px] text-neutral-400 mt-1">Container listening port (e.g. 3000, 8080)</p>
            </div>
        </div>
        ...
        {form.dbType !== 'none' && (
            <div className="p-4 rounded-xl bg-neutral-900 text-neutral-200 border border-neutral-700 shadow-inner">
                <div className="flex items-center gap-2 mb-2 text-green-400">
                    <Database size={16} />
                    <span className="text-xs font-bold uppercase tracking-wider">Internal Database DNS</span>
                </div>
                <div className="flex items-center gap-2 bg-black/30 p-2 rounded border border-white/10">
                    <code className="text-sm font-mono flex-1 text-white">
                        svc-{form.appName}-db-{nextId}
                    </code>
                    <button 
                        type="button"
                        onClick={() => copyToClipboard(`svc-${form.appName}-db-${nextId}`)}
                ...
                <p className="text-[10px] text-neutral-500 mt-2">
                    Use this hostname to connect to your database from within the cluster. Port: <strong>{form.dbType === 'postgres' ? '5432' : '3306'}</strong>.
                </p>
            </div>
        )}

        {/* Secrets Section (Simplified View for Review) */}
        <div className="border-t border-neutral-200 dark:border-neutral-800 pt-4">
             <div className="flex items-center justify-between mb-4">
                 <h3 className="font-bold text-sm text-neutral-600 dark:text-neutral-300">Application Secrets</h3>
                 <button type="button" onClick={addAppSecret} className="text-xs font-bold text-[#FFA500] flex items-center gap-1"><Plus size={12}/> Add</button>
             </div>
             {/* Simple List or DragDrop reused? Let's just use the Secret inputs here */}
             <div className="space-y-2 max-h-[200px] overflow-y-auto pr-2">
                 {form.appSecrets.map((secret, idx) => (
                     <div key={idx} className="flex gap-2">
                        <input className="flex-1 p-2 text-xs border rounded dark:bg-neutral-950 dark:border-neutral-800 font-mono" value={secret.key} onChange={e => updateAppSecret(idx, 'key', e.target.value)} placeholder="KEY" />
                        <input className="flex-1 p-2 text-xs border rounded dark:bg-neutral-950 dark:border-neutral-800" value={secret.value} onChange={e => updateAppSecret(idx, 'value', e.target.value)} placeholder="VALUE" />
                     </div>
                 ))}
         ...
         {/* DB Secrets (If DB selected) */}
         {form.dbType !== 'none' && (
             <div className="border-t border-neutral-200 dark:border-neutral-800 pt-4">
                 <h3 className="font-bold text-sm text-neutral-600 dark:text-neutral-300 mb-2">Database Secrets</h3>
                 <div className="space-y-2">
                     {form.dbSecrets.map((secret, idx) => (
                         <div key={idx} className="flex gap-2">
                            <input className="flex-1 p-2 text-xs border rounded dark:bg-neutral-950 dark:border-neutral-800 font-mono bg-neutral-100 dark:bg-neutral-800" value={secret.key} readOnly />
                            <input className="flex-1 p-2 text-xs border rounded dark:bg-neutral-950 dark:border-neutral-800" value={secret.value} onChange={e => updateDbSecret(idx, 'value', e.target.value)} placeholder="VALUE" />
                         </div>
                     ))}
                 </div>
             </div>
  ...
  return (
    <div className="bg-white dark:bg-neutral-900 w-full rounded-xl border border-neutral-200 dark:border-neutral-800 shadow-sm flex flex-col relative overflow-hidden">
       ...
       <div className="flex-1 overflow-y-auto p-8 relative">
           {/* Messages */}
            {message.text && (
            <div className={`mb-6 p-4 rounded-lg border flex items-center gap-3 text-sm animate-in slide-in-from-top-2 ${ 
